...
class AtomEmbedding(nn.Module):
    def __init__(
        ...
        # d_atom_feats = 137; atom's feature dimension
...
class TripletEmbedding(nn.Module):
    def __init__(
        ...
        # here
        self.cf_proj = MLP(d_cf_feats, d_g_feats, 2, activation)
        self.md_proj = MLP(d_md_feats, d_g_feats, 2, activation)
    def forward(self, node_h, edge_h, fp, cf, md, indicators):
        ...
        # here
        triplet_h[indicators==3] = self.cf_proj(cf)
        return triplet_h
...
class LiGhTPredictor(nn.Module):
    def __init__(self,
        ...
        self.triplet_emb = TripletEmbedding(d_g_feats, d_fp_feats, d_cf_feats, d_md_feats, activation)
        # here
        self.mask_emb = nn.Embedding(1, d_g_feats)
        # Model
        self.model = LiGhT(
        ...
        self.cf_predictor = nn.Sequential(
            nn.Linear(d_g_feats, d_g_feats),
            activation,
            nn.Linear(d_g_feats, d_cf_feats)
        )
    ...
    def forward(self, g, fp, cf, md, idx):
        # ``g'' comes from data/featurizer.py
        # uses dgl.graph(data), where ``data'' contains the information of atom and bond
        # `vavn' means virtual_atom_and_virtual_node_labels
        indicators = g.ndata['vavn'] # 0 indicates normal atoms and nodes (triplets); -1 indicates virutal atoms; >=1 indicate virtual nodes 
        # Input
        # 'begin_end' : [atom_pairs_features_in_triplets], eg. [A1's feats, A2's feats]
        node_h = self.node_emb(g.ndata['begin_end'], indicators)   # (num_atom_pairs, 768)
        # 'edge' : [Bond's feat]       
        edge_h = self.edge_emb(g.ndata['edge'], indicators)    # (num_edges, 768)
        
        triplet_h = self.triplet_emb(node_h, edge_h, fp, cf, md, indicators)   # (num_triplet, 768)
        # here
        # model & predict
        triplet_h[g.ndata['mask']==idx] = self.mask_emb.weight
        triplet_h = self.model(g, triplet_h)
        node_pre = self.node_predictor(triplet_h[g.ndata['mask']>=1])
        fp_pre = self.fp_predictor(triplet_h[indicators==1])   # (128, 512)
        cf_pre = self.cf_predictor(triplet_h[indicators==3])
        md_pre = self.md_predictor(triplet_h[indicators==2])

        return triplet_h, node_pre, fp_pre, cf_pre, md_pre
