...
class Collator_pretrain(object):
    def __init__(
        ...
        candi_rate=0.15, mask_rate=0.25, replace_rate=0.1, keep_rate=0.15,
        fp_disturb_rate=0.15, md_disturb_rate=0.15, cf_disturb_rate=0.15 # here
        ):
        self.vocab = vocab
        ...
        # here
        self.cf_disturb_rate = cf_disturb_rate
    ...
    def bert_mask_nodes(self, g):
        ...
        mask_ids_1 = np.random.choice(candi_ids, size=int(len(candi_ids)*self.mask_rate),replace=False)
        candi_ids = np.setdiff1d(candi_ids, mask_ids_1)
        mask_ids_2 = np.random.choice(candi_ids, size=int(len(candi_ids)*(self.mask_rate-0.05)),replace=False)
        candi_ids = np.setdiff1d(candi_ids, mask_ids_2)
        mask_ids_3 = np.random.choice(candi_ids, size=int(len(candi_ids)*(self.mask_rate+0.05)),replace=False)
        candi_ids = np.setdiff1d(candi_ids, mask_ids_3)
        ...
        g.ndata['mask'][mask_ids_1] = 1
        g.ndata['mask'][mask_ids_2] = 2
        g.ndata['mask'][mask_ids_3] = 3
        g.ndata['mask'][replace_ids] = 4
        g.ndata['mask'][keep_ids] = 5
        sl_labels = g.ndata['label'][g.ndata['mask']>=1].clone()
    ...
    def disturb_cf(self, cf):
        cf = deepcopy(cf)
        b, d = cf.shape
        cf = cf.reshape(-1)
        disturb_ids = np.random.choice(b*d, int(b*d*self.cf_disturb_rate), replace=False)
        cf[disturb_ids] = 1 - cf[disturb_ids]
        return cf.reshape(b,d)
    ...
    def __call__(self, samples):
        smiles_list, fps, cfs, mds = map(list, zip(*samples))
        graphs = []
        ...
        cfs = torch.stack(cfs, dim=0).reshape(len(smiles_list),-1)
        ...
        disturbed_cfs = self.disturb_cf(cfs)
        disturbed_mds = self.disturb_md(mds)
        return smiles_list, batched_graph, fps, cfs, mds, sl_labels, disturbed_fps, disturbed_cfs, disturbed_mds

class Collator_tune(object):
    def __init__(self, max_length=5, n_virtual_nodes=3, add_self_loop=True):
        self.max_length = max_length
        self.n_virtual_nodes = n_virtual_nodes
        self.add_self_loop = add_self_loop
    def __call__(self, samples):
        smiles_list, graphs, fps, cfs, mds, labels = map(list, zip(*samples))
        batched_graph = dgl.batch(graphs)
        fps = torch.stack(fps, dim=0).reshape(len(smiles_list),-1)
        cfs = torch.stack(cfs, dim=0).reshape(len(smiles_list),-1)
        mds = torch.stack(mds, dim=0).reshape(len(smiles_list),-1)
        ...
        batched_graph.edata['path'][:, :] = preprocess_batch_light(batched_graph.batch_num_nodes(), batched_graph.batch_num_edges(), batched_graph.edata['path'][:, :])
        return smiles_list, batched_graph, fps, cfs, mds, labels
