...
class MoleculeDataset(Dataset):
    def __init__(self, root_path, dataset, dataset_type, path_length=5, n_virtual_nodes=3, split_name=None, split=None):
        ...
        cf_path = os.path.join(root_path, f"{dataset}/chemical_features.npz")
        ...
        cfs = torch.from_numpy(sps.load_npz(cf_path).todense().astype(np.float32))
        mds = np.load(md_path)['md'].astype(np.float32)
        mds = torch.from_numpy(np.where(np.isnan(mds), 0, mds))
        # here
        self.df, self.fps, self.cfs, self.mds = df.iloc[use_idxs], fps[use_idxs], cfs[use_idxs], mds[use_idxs]
        ...
        self.d_cfs = self.cfs.shape[1]  # here
    ...
    def _pre_process(self):
        ...
        self.fps, self.cfs, self.mds = self.fps, self.cfs, self.mds
    def __len__(self):
        return len(self.smiless)
    
    def __getitem__(self, idx):
        return self.smiless[idx], self.graphs[idx], self.fps[idx], self.cfs[idx], self.mds[idx], self.labels[idx]
