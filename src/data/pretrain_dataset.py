...
class MoleculeDataset(Dataset):
    def __init__(self, root_path):
        ...
        cf_path = os.path.join(root_path, "chemical_features.npz")
        ...
        self.cfs = torch.from_numpy(sps.load_npz(cf_path).todense().astype(np.float32))
        ...
        self.d_cfs = self.cfs.shape[1]
        self.d_mds = self.mds.shape[1]        
        
        self._task_pos_weights = self.task_pos_weights()
        self._cf_pos_weights = self.cf_pos_weights()
    def __len__(self):
        return len(self.smiles_list)
    
    def __getitem__(self, idx):
        return self.smiles_list[idx], self.fps[idx], self.cfs[idx], self.mds[idx]
    ...
    def cf_pos_weights(self):
        task_pos_weights = torch.ones(self.cfs.shape[1])
        num_pos = torch.sum(torch.nan_to_num(self.cfs,nan=0), axis=0)
        masks = F.zerocopy_from_numpy(
            (~np.isnan(self.cfs.numpy())).astype(np.float32))
        num_indices = torch.sum(masks, axis=0)
        task_pos_weights[num_pos > 0] = ((num_indices - num_pos) / num_pos)[num_pos > 0]
        return task_pos_weights