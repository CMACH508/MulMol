...
class Vocab(object):
    ...
    def index(self, atom_type1, atom_type2, bond_type):
        # finds the id of triplet
        # if not find, then return size of vocabs
...
def smiles_to_graph(smiles, vocab, max_length=5, n_virtual_nodes=8, add_self_loop=True):
    ...
    ## bonded atoms
    triplet_labels = []  # locate vocab id using vocab.index()
    ...
    for bond in mol.GetBonds():
        # traverse all bonds, if there has a bond, then there has a A-B-A triplet
        ...
        # `atom_features[begin_atom_id][:N_ATOM_TYPES].index(1)` finds the atom type
        # `vocab.index(a1, a2, b1)` finds the index of triplet
        ...
        # append(0) means if this label==0, then the triplet has real A-B-A
    ...
    for atom_id in range(n_atoms):
        if atom_id not in bonded_atoms:
            ...
            # virtual atom & bond has Type id 999
            triplet_labels.append(vocab.index(atom_features[atom_id][:N_ATOM_TYPES].index(1),999,999))
            #append(VIRTUAL_ATOM_INDICATOR=-1) means if this label==-1, then the triplet is an unbonded atom
    ...
    for i in range(n_atoms):
        # 'node_ids' contains all triplets that include node i
        node_ids = atomIDPair_to_tripletId[i]
        node_ids = node_ids[~np.isnan(node_ids)]
        if len(node_ids) >= 2:
            # permutations: Generate all permutations of length 2
    ...
    # molecule's Adjacency matrix(邻接矩阵)
