...
sigFactory.SetBins([(0,2), (2,4), (4,8)])
sigFactory.Init()
...
def preprocess_dataset(args):
    ...
    for smiles in smiless:
        mol = Chem.MolFromSmiles(smiles)
        cf_list.append(AllChem.GetMorganFingerprintAsBitVect(mol,2,nBits=256))
    cf_arr = np.array(cf_list)
    cf_sp_mat = sp.csc_matrix(cf_arr)
    print('saving chemical features')
    sp.save_npz(f"{args.data_path}/{args.dataset}/chemical_features.npz", cf_sp_mat)
