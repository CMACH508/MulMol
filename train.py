...
if __name__ == '__main__':
    ...
    collator = Collator_pretrain(vocab, max_length=config['path_length'], n_virtual_nodes=3, candi_rate=config['candi_rate'], fp_disturb_rate=config['fp_disturb_rate'], md_disturb_rate=config['md_disturb_rate'], cf_disturb_rate=config['cf_disturb_rate'])
    ...
    model = LiGhT(
        ...
        d_cf_feats=train_dataset.d_cfs,
    ...
    clf_loss_fn_cf = BCEWithLogitsLoss(weight=train_dataset._cf_pos_weights.to(device),reduction='none')
    ...
    clf_evaluator_fp = Evaluator("chembl29", clf_metric, train_dataset.d_fps)
    clf_evaluator_cf = Evaluator("chembl29", clf_metric, train_dataset.d_cfs)
    result_tracker = Result_Tracker(reg_metric)
    if local_rank == 0:
        summary_writer = SummaryWriter(f"tensorboard/pretrain-{args.config}", )
    else: 
        summary_writer = None
    trainer = Trainer(args, optimizer, lr_scheduler, reg_loss_fn, clf_loss_fn, clf_loss_fn_cf, sl_loss_fn, reg_evaluator, clf_evaluator_fp, clf_evaluator_cf, result_tracker, summary_writer, device=device,local_rank=local_rank)
